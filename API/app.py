...
from API import angles, processing_video
...
@app.route('/coach/add_shot_result/<int:sessionshot_id>', methods=["GET"])
def add_shot_result(sessionshot_id):
    data = request.get_json()
    required_field = "video_path"
    if required_field not in data:
        return jsonify({f"error: required field f{required_field} is missing from the data"})
    try:

        my_angles = processing_video.process_video(data["video_path"])

        results = CoachController.add_shot_result(sessionshot_id, my_angles)
